...
def add_order(conn, order_id, client_order_id, symbol="AAPL", side="buy", status="filled"):
    conn.execute(
...
def add_fill(conn, activity_id, order_id, symbol="AAPL", side="buy", qty="10", price="100", at=None):
    conn.execute(
...
def arrange_buy(conn, *, qty="10", client_order_id="kt-mom-1", order_id="o1",
                activity_id="act-1", symbol="AAPL", at=None):
...
def test_preparing_the_same_rebalance_twice_attributes_the_same_orders(conn):
    ...
    again = [attribute(conn, "kt-mom_12_1-aaa", symbol="AAPL"),
             attribute(conn, "kt-mom_12_1-bbb", symbol="MSFT")]
...
def test_attributed_strategies_are_listed_for_the_account(conn):
    ...
    assert attributed_strategies(conn, broker_account_id=ACCOUNT) == sorted([MOM, OTHER])
...
def test_a_plan_is_attributed_without_persisting_any_quantity(conn, tmp_path):
    ...
    with path.open("w", newline="", encoding="utf-8") as handle:
        ...
        writer.writerow(["symbol", "weight", "signal_date", "intended_execution_date",
                         "strategy", "strategy_version", "universe_hash", "selected_count"])
        for symbol in symbols:
            writer.writerow([symbol, "0.333333333", "2026-07-31", "2026-08-03",
                             MOM, "1.0.0", "f7b50c2b0c0882df", 3])
    plan = build_rebalance_plan(
        ...
        asset_facts={s: AssetFact(symbol=s, tradable=True, fractionable=True) for s in symbols},
        ownership=StrategyOwnershipLedger(strategy=MOM, positions={}, available=True, source="t"),
        reconciliation=ReconciliationEvidence(
            run_id=1, status="clean", completed_at=datetime.now(UTC) - timedelta(minutes=1),
            broker_account_id=ACCOUNT,
        ),
    )
...
def test_requested_quantity_is_never_read(conn):
    ...
    assert "filled_quantity" not in source.split("CREATE")[0].split("broker_orders o")[0]
...
def test_the_unique_constraint_is_what_stops_a_double_apply(conn):
    ...
    with pytest.raises(sqlite3.IntegrityError):
        conn.execute(
            ...
            (ACCOUNT, MOM, "AAPL", "act-1", "o1", "kt-mom-1", "buy",
             "10", "100", "10", "20", T0, T0),
        )
...
def test_drift_blocks_further_writes_rather_than_being_overwritten(conn):
    ...
    conn.commit()
    conn.execute(
        "UPDATE strategy_owned_positions SET quantity=%s WHERE symbol=%s", ("999", "AAPL")
    )
    conn.commit()
...
def test_replay_ignores_the_stored_aggregate_entirely(conn):
    ...
    replayed = replay_ownership_from_fills(conn, broker_account_id=ACCOUNT, strategy=MOM)
    assert replayed.owned_quantity("AAPL") == Decimal(10)
    report = verify_ownership_against_replay(conn, broker_account_id=ACCOUNT, strategy=MOM)
...
def test_broker_sync_applies_ownership_inside_its_transaction():
    ...
    assert cycle.index("persist_normalized_state(") < cycle.index("conn.commit()", cycle.index("persist_normalized_state("))
