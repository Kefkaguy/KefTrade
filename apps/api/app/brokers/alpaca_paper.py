...
class AlpacaPaperBrokerAdapter:
    ...
    async def _get(
        self, path: str, endpoint_class: str, params: dict[str, Any] | None = None
    ) -> BrokerResponse:
        client = self._client()
    ...
    async def _get_all_pages(
        ...
        *,
        ...
        """Read an Alpaca activity collection through its final page.

        Alpaca uses the last activity id as ``page_token``. A repeated token or
        a full page without an id is a malformed pagination response and fails
        the entire broker sync; silently accepting a prefix would make the fill
        ledger look complete when it is not.
        """
        client = self._client()
        owns_client = self._provided_client is None
        ...
        try:
            while True:
                ...
                if not isinstance(page, list):
                    raise TypeError(
                        f"{endpoint_class} pagination returned a non-list payload"
                    )
                payload.extend(dict(item) for item in page)
                if len(page) < page_size:
                    break
                token = str((page[-1] or {}).get("id") or "")
                if not token or token in seen_tokens:
                    raise RuntimeError(f"{endpoint_class} pagination did not advance")
    ...
    async def list_orders(self) -> BrokerResponse:
        return await self._get(
            "/v2/orders",
            "orders",
            {"status": "all", "limit": 500, "direction": "asc", "nested": "false"},
        )
    ...
    async def submit_order(self, payload: dict[str, Any]) -> BrokerResponse:
        ...
        if str(payload.get("side") or "").lower() != "buy":
            raise BrokerMutationDisabled(
                "KefTrade external paper supports buy orders only"
            )
        if (
            not settings.broker_order_submission_enabled
            or not settings.external_paper_execution_enabled
        ):
            raise BrokerMutationDisabled(
                "both broker execution flags must be enabled for Alpaca Paper mutation"
            )
    ...
    async def cancel_order(self, broker_order_id: str) -> BrokerResponse:
        ...
        return await self._mutate(
            "DELETE", f"/v2/orders/{broker_order_id}", "cancel_order"
        )
    ...
    async def _submit_governed_order(
        ...
        *,
        ...
        await self._revalidate_reduction_now(
            wire, confirmed_positions, evidence, ledger
        )
